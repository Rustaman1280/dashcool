...
<body class="min-h-full flex flex-col bg-[#FAF9F6] text-slate-800 selection:bg-slate-900 selection:text-white pb-20 md:pb-8">
    ...
    <!-- TOP ACCESSIBLE APP BAR -->
    <header class="no-print sticky top-0 z-40 bg-white/90 backdrop-blur-md text-slate-900 border-b border-slate-200/80 shadow-sm">
        <div class="max-w-2xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
            
            <!-- School Brand / Logo -->
            <a href="{{ route('spmb.public.register') }}" class="flex items-center gap-3 transition-opacity hover:opacity-90">
                <div class="w-10 h-10 rounded-xl bg-slate-100 p-1 flex items-center justify-center border border-slate-200 flex-shrink-0">
                    <img src="{{ asset('logo.webp') }}" class="w-8 h-8 object-contain" alt="Logo Dashcool">
                </div>
                <div>
                    <div class="flex items-center gap-2">
                        <span class="font-extrabold text-base tracking-tight text-slate-900">SPMB ONLINE</span>
                        <span class="text-xs font-bold uppercase tracking-wider bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full">BUKA</span>
                    </div>
                    <p class="text-xs text-slate-500 font-medium leading-tight mt-0.5">SMA Dashcool Nusantara</p>
                </div>
            </a>
            ...
            <div class="flex items-center gap-2">
                <a href="https://wa.me/6281234567890?text=Halo%20Panitia%20SPMB%2C%20saya%20ingin%20bertanya%20mengenai%20pendaftaran%20siswa%20baru" 
                   ...
                   class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-sm transition-colors">
    ...
    <main class="flex-1 w-full max-w-2xl mx-auto px-4 py-4 sm:py-6">
        ...
        @if (session('success'))
            <div class="mb-4 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-sm flex items-start gap-3 shadow-sm">
        ...
        @if (session('error'))
            <div class="mb-4 p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-900 text-sm flex items-start gap-3 shadow-sm">
                <div class="w-6 h-6 rounded-full bg-rose-600 text-white flex items-center justify-center flex-shrink-0 mt-0.5">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <nav class="no-print fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur-lg border-t border-slate-200/80 shadow-md md:hidden">
        <div class="max-w-2xl mx-auto px-4 h-16 grid grid-cols-3 items-center">
            ...
            <a href="{{ route('spmb.public.register') }}" 
               class="flex flex-col items-center justify-center gap-1 py-1.5 transition-colors {{ request()->routeIs('spmb.public.register') || request()->routeIs('spmb.public.success') ? 'text-slate-900 font-bold' : 'text-slate-500 hover:text-slate-900' }}">
                <div class="relative">
                ...
                <span class="text-xs font-semibold tracking-tight">Daftar SPMB</span>
            </a>

            <!-- 2. Cek Status Seleksi -->
            <a href="{{ route('spmb.public.status') }}" 
               class="flex flex-col items-center justify-center gap-1 py-1.5 transition-colors {{ request()->routeIs('spmb.public.status') ? 'text-slate-900 font-bold' : 'text-slate-500 hover:text-slate-900' }}">
                <div class="relative">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                <span class="text-xs font-semibold tracking-tight">Cek Status</span>
            </a>

            <!-- 3. Bantuan & Panduan -->
            <a href="https://wa.me/6281234567890?text=Halo%20Panitia%20SPMB%2C%20saya%20membutuhkan%20informasi%20syarat%20pendaftaran" 
               target="_blank"
               class="flex flex-col items-center justify-center gap-1 py-1.5 text-slate-500 hover:text-emerald-700 transition-colors">
                ...
                <span class="text-xs font-semibold tracking-tight">Bantuan WA</span>
            </a>
    ...
    <!-- FOOTER DESKTOP -->
    <footer class="no-print max-w-2xl mx-auto px-4 text-center mt-auto pt-6 text-xs text-slate-400">
